...
# =======================================================================
# PART 3: FINAL, MOST CHALLENGING REASONING TASKS
# =======================================================================
...
# --- Test 16: Crafting Chain & Resource Hierarchy ---
class CraftingChainDataset(BaseTaskDataset):
    """
    Tests managing a multi-level inventory and executing multi-step plans.
    """
    def __init__(self, num_samples, seq_len):
        super().__init__(num_samples, seq_len)
        self.vocab = {'PAD': 0, 'GATHER_WOOD': 1, 'GATHER_ORE': 2, 'CRAFT_HANDLE': 3, 'SMELT_INGOT': 4, 'CRAFT_AXE': 5}
        self.recipes = {
            'CRAFT_HANDLE': {'WOOD': 2},
            'SMELT_INGOT': {'ORE': 3},
            'CRAFT_AXE': {'HANDLE': 1, 'INGOT': 2}
        }
    ...
    def _generate_data(self):
        ...
        for _ in range(self.num_samples):
            seq, lab = [], []
            inventory = {'WOOD': 0, 'ORE': 0, 'HANDLE': 0, 'INGOT': 0}
            for _ in range(self.seq_len):
                action = random.choice(list(self.vocab.keys())[1:])
                seq.append(self.vocab[action])

                if action.startswith('GATHER'):
                    resource = action.split('_')[1]
                    inventory[resource] += 1
                    lab.append(0) # No prediction needed for gathering
                
                elif action.startswith('CRAFT') or action.startswith('SMELT'):
                    recipe = self.recipes[action]
                    can_craft = all(inventory[res] >= count for res, count in recipe.items())
                    ...
                    if can_craft:
                        ...
                        if action == 'CRAFT_HANDLE': inventory['HANDLE'] += 1
                        elif action == 'SMELT_INGOT': inventory['INGOT'] += 1
                    else:
                        lab.append(2) # FAIL
...
# --- Test 17: 2D Dynamic Pathfinding ---
class Pathfinding2DDataset(BaseTaskDataset):
    """
    Tests tracking positions in a 2D space where one agent's movement
    is a non-trivial function of the other's past movement.
    """
    def __init__(self, num_samples, seq_len, grid_size=8):
        self.grid_size = grid_size
        super().__init__(num_samples, seq_len)
        self.vocab = {'PAD': 0, 'AGENT_UP': 1, 'AGENT_DOWN': 2, 'AGENT_LEFT': 3, 'AGENT_RIGHT': 4}
        self.num_classes = grid_size * grid_size # Predict one of the grid cells
        self.sequences, self.labels = self._generate_data()

    def _generate_data(self):
        sequences, labels = [], []
        moves = {'AGENT_UP': [0, 1], 'AGENT_DOWN': [0, -1], 'AGENT_LEFT': [-1, 0], 'AGENT_RIGHT': [1, 0]}

        for _ in range(self.num_samples):
            seq, lab = [], []
            agent_pos = [self.grid_size // 2, self.grid_size // 2]
            target_pos = [self.grid_size // 2, self.grid_size // 2]
            last_agent_move = [0, 0] # [dx, dy]
            
            for _ in range(self.seq_len):
                # Target moves based on agent's LAST move (opposite direction)
                target_move = [-last_agent_move[0], -last_agent_move[1]]
                target_pos[0] += target_move[0]
                target_pos[1] += target_move[1]
                
                # Clip target position to grid boundaries
                target_pos[0] = max(0, min(target_pos[0], self.grid_size - 1))
                target_pos[1] = max(0, min(target_pos[1], self.grid_size - 1))
                
                # The label is the flattened 2D coordinate of the target
                lab.append(target_pos[1] * self.grid_size + target_pos[0])
                
                # Agent makes its CURRENT move
                action = random.choice(list(moves.keys()))
                seq.append(self.vocab[action])
                current_move = moves[action]
                
                agent_pos[0] += current_move[0]
                agent_pos[1] += current_move[1]

                # Clip agent position
                agent_pos[0] = max(0, min(agent_pos[0], self.grid_size - 1))
                agent_pos[1] = max(0, min(agent_pos[1], self.grid_size - 1))
                
                last_agent_move = current_move
...
def train_and_evaluate(model, model_name, train_loader, test_loader, epochs, lr, device, num_classes):
    model.to(device)
    # Using ignore_index=0 since PAD is always 0
...
if __name__ == '__main__':
    config = {
        ...
        'epochs': 10, # These are the hardest tasks, more epochs are recommended
        'lr': 0.0005, # A slightly lower learning rate can help with stability on complex tasks
        'device': "cuda" if torch.cuda.is_available() else "cpu",
        'num_samples_train': 12000, 'num_samples_test': 2000,
        'embed_dim': 128, 'hidden_dim': 512, 'num_heads': 4, 'num_layers': 4,
    }

    all_tasks = {
        "16_crafting_chain": CraftingChainDataset,
        "17_dynamic_pathfinding_2d": Pathfinding2DDataset,
    }

    # #################################################
    # ### CHOOSE YOUR FINAL BOSS EXPERIMENT HERE ###
    # #################################################
    
    task_to_run = "17_dynamic_pathfinding_2d"
